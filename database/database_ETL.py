...
class DatabaseManager:
    ...
    def upsert_data(self, query: dict, data: dict):
        ...
        if existing_record:
            ...
            print(
                f"Data in {self.collection_name} collection in {self.db_name} database updated."
            )
        else:
            collection.insert_one(data)
            print(
                f"Data inserted into {self.collection_name} collection in {self.db_name} database."
            )
...
if __name__ == "__main__":
    ...
    company_info = {
        ...
        "parameters": {"tag": "h3", "attribute": {"class": "title"}, "index_num": 0},
        "filters": [],
        "is_local": True,
